...
JoinGraphics::JoinGraphics(Ogre::ManualObject* manualObject, Ogre::SceneNode* sceneNode, DrawingAlgorithm* drawingAlgorihtm) : ElementGraphics(manualObject, sceneNode, drawingAlgorihtm) {}

JoinGraphics::~JoinGraphics() {}

Ogre::Vector3 JoinGraphics::getConnectorBeginPoint(const std::string& A_name)
{
	DEFL;
	Element* target = Utilities::getConnectorTarget(NULL);
	...
	double thisSize = static_cast<JoinAlgorithm*>(this->drawingAlgorihtm)->getFullSize();
...
Ogre::Vector3 JoinGraphics::getConnectorEndPoint(const std::string& A_name)
{
